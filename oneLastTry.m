...
I_1 = log(abs(I_1));
min_I = min(I_1(:)); I_1 = I_1 - min_I;
max_I = max(I_1(:)); I_1 = I_1 / max_I;

I_2 = fftshift(fft2(i_2, FFT_Prec, FFT_Prec));
I_2 = log(abs(I_2));
min_I = min(I_2(:)); I_2 = I_2 - min_I;
max_I = max(I_2(:)); I_2 = I_2 / max_I;


%Images in polar coordonates
I_1_PC = imgpolarcoord(I_1);
I_2_PC = imgpolarcoord(I_2);
